...
//			}
	...
	public boolean remoteExecute(final Task task, final Integer exec_type) throws Exception{
		...
		try {
			...
			//修复一个严重的bug，当远程执行的时候，一开始的undoReport表是从远端的daemon程序以http post的方式存入undo表,
			//	然后再以方法调用的结果作为返回，如果执行一半网络断掉，这样就无法获得远端的执行结果了
			...
			record.setExec_return_str("无法ping通daemon端[ip:" + task.getDaemon().getMachine_ip() + 
					",port:" + task.getDaemon().getMachine_port() + "],执行命令:" + task.getShell_cmd() + "的时候无法通信");
			record.setCurrent_redo_times(0);
			record.setOn_processing(false);
			doneRecordDao.insert(record);
			AppLogger.errorLogger.error("error in execute exec_type:" + Params.EXECTYPE_REPRESENT.get(exec_type) + 
					" . ip:" + task.getDaemon().getMachine_ip() + 
					",cmd:" + task.getShell_cmd()+".",e);
			throw new Exception(Params.PAGE_CONN_LOST_MSG+",task_id:" + task.getId() + 
					",服务器ip:" + machine_ip + ",端口号:" + machine_port);
		}finally{
			//如果已经汇报到未完成表了,那么Map中必然有,删除之
			if(Params.REPORT_UNDO_IDENTIFIER_ID.containsKey(report_undo_identifier)){
				undoRecordDao.deleteById(Params.REPORT_UNDO_IDENTIFIER_ID.get(report_undo_identifier));
		...
		doneRecordDao.insert(record);
		undoRecordDao.deleteById(result.getTask_record_undo_id());
		AppLogger.recordDoneLogger.info(String.format("execute type:%s done!ip:%s," +
				"port:%s.insert to record_done table.[start_datetime:%s,end_datetime:%s].Delete" +
				" from record_undo table.record_undo_id=%s", 
				Params.EXECTYPE_REPRESENT.get(exec_type),
				task.getDaemon().getMachine_ip(), 
				task.getDaemon().getMachine_port(),
				sdf.format(record.getStart_datetime()),
				sdf.format(record.getEnd_datetime()),
				result.getTask_record_undo_id()));
