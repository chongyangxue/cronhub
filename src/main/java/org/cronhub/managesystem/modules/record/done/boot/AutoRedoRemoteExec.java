...
public class AutoRedoRemoteExec  extends ContextLoaderListener{
	...
	private String allRedoWhereSql = " WHERE on_processing = 0 " +
				" AND (exec_type = " + Params.EXECTYPE_CRONTAB + 
				" OR exec_type = " + Params.EXECTYPE_AUTOREDO + ")" +
				" AND exit_code!=" + Params.DB_EXITCODE_ERROR_PING + 
				" AND conn_status = 1 AND complete_success = 0 " +
				" AND is_redo = 1 AND current_redo_times < end_redo_times ";
	...
	public void contextInitialized(ServletContextEvent event) {
		AutoRedoRemoteExec exec = (AutoRedoRemoteExec)WebContainer.
				getBean("autoRedoRemoteExec",event.getServletContext());
		exec.start();
	}
						...
						table.add(exec_cron_exp, new Task(){
							...
							public void execute(TaskExecutionContext context){
								try {
									...
									AppLogger.recordDoneErrorLogger.error(
										"自动重执行task_record_done表的记录时失败[表名:" +	tableName 
										+ ",task_record_done的id:" + record.getId(), e);
								}
							}
						});
					}
