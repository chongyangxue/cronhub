...
public class DoneRecordDaoImpl implements IDoneRecordDao {
	...
	public List<TaskRecordDone> findByPage(String tableName,String orderLimit, FillConfig fillConfig) {
		String sql = String.format("SELECT %s.id AS id, task_id, complete_success," +
				"exit_code, daemon.machine_ip AS machine_ip, daemon.conn_status AS conn_status," +
				"task.cron_exp AS cron_exp, real_cmd, start_datetime, end_datetime," +
				"task.run_mode AS run_mode, exec_type, task.is_process_node AS is_process_node," +
				"task.is_redo AS is_redo, current_redo_times, task.end_redo_times AS end_redo_times," +
				"on_processing FROM (%s LEFT JOIN task ON %s.task_id = task.id) " +
				"LEFT JOIN daemon ON task.daemon_id = daemon.id %s", tableName, tableName, tableName, orderLimit);
		...
		if(fillConfig.getFillTask()){
			for(TaskRecordDone done : taskRecordDones){
				...
				if(fillConfig.getFillDaemon() && task != null){
