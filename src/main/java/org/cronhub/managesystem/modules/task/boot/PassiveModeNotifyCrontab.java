...
public class PassiveModeNotifyCrontab extends ContextLoaderListener{
			...
			sch.addTaskCollector(new TaskCollector(){
				...
				public TaskTable getTasks() {
					...
					for(final Task task : tasks){
						table.add(new SchedulingPattern(task.getCron_exp()), 
							new it.sauronsoftware.cron4j.Task(){
								@Override
								public void execute(TaskExecutionContext context) throws RuntimeException {
									try {
										...
										System.err.println("task is failed while executing task:" + task.getId());
									}
								}
							});
					}
	...
	private Runnable crontabProcessThread = new Runnable() {
		@Override
		public void run() {
			Scheduler sch = new Scheduler();
			sch.addTaskCollector(new TaskCollector(){
				...
				public TaskTable getTasks() {
					...
					for(final Process process : processes){
						final List<Task> tasks = processDao.findTasks(process.getId());
						final int taskCount = tasks.size();
						table.add(new SchedulingPattern(process.getCron_exp()), 
							new it.sauronsoftware.cron4j.Task(){
								@Override
								public void execute(TaskExecutionContext context) throws RuntimeException {
									...
									while(queueCount < taskCount) {
										for(Iterator<Task> iter = tasks.iterator(); iter.hasNext();) {
											...
											for(Task queue : taskQueue) {
												System.out.println("task_id: "+ task.getId() + " | pre_id: " + task.getPre_task_id());
									...
									try {
										for(Task task : taskQueue) {
											boolean isSuccess = processor.remoteExecute(task, Params.EXECTYPE_CRONTAB);
											if(!isSuccess)
												break;
										}
									}catch (Exception e) {
										e.printStackTrace();
										throw new RuntimeException("task is failed while executing task.");
									}
								}
							});
					}
...
//									}
	...
	private void startProcessThread(){
		Thread processThread = new Thread(this.crontabProcessThread);
	...
	public void contextInitialized(ServletContextEvent event) {
		PassiveModeNotifyCrontab cron = (PassiveModeNotifyCrontab)WebContainer
				.getBean("passiveModeNotifyCrontab", event.getServletContext());
		cron.startCrontabThread();
		cron.startProcessThread();
	}
